...
public class UpdateQueryImpl extends BaseQuery<UpdateQuery> implements UpdateQuery{
    public UpdateQueryImpl(String sql){
        super(sql);
    }

    public UpdateQueryImpl(String sql, DbConfig dbConfig){
        super(sql, dbConfig);
    }
    ...
    public int execute() {
        ...
        //update
        PnxSteps.start(parsedSql.getSql(), "SQL", "Update");
        String subKey = parsedSql.getSql().substring(0, 6).toLowerCase();
        if(!subKey.equalsIgnoreCase("update")){
